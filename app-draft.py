...
def user_input_features():
    ...
    inputs = {
        ...
        'Flight Distance': st.sidebar.number_input('Flight Distance ✈️', 0, 1000, 100),
...
# Function make predictions and explain the results
# def make_predictions(model, preprocessor, input, df, explainer, lime_explainer):
#     try:
...
#             shap_values = shap_values[1] # Assuming binary class, use the second class 1 (Satisfied) 0 for (Not Satisfied)
        
#         # LIME explanation
#         lime_explanation = lime_explainer.explain_instance(
#             data_row=input_df_transformed[0],  # [0] first value
...
def make_predictions(model, preprocessor, input_df, explainer, lime_explainer):
    try:
        ...
        return None, None, None, None, None
        # Why 5? for 1-Satisfied/Not Satisfied, 2-shap_values, 3-explainer.expected_value, 4-input_df_transformed, 5-lime_explanation
...
#     result, shap_values, explainer.expected_value, input_df_transformed, lime_explanation = make_predictions(selected_model, preprocessor, input_df, explainer, lime_explainer)
